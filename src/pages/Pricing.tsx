...
export default function PricingPage() {
  ...
  const handleCheckoutInit = async (phone: { number: string; country_code: string }) => {
    ...
    try {
      console.log('🟡 Pricing: Initializing Chariow checkout');

      // Utiliser le service Chariow
      ...
      toast.success('Prêt !', {
        description: 'Vous allez être redirigé vers Chariow.'
      });
      ...
      toast.error('Paiement indisponible', {
        description: err?.message || "La session de paiement n'a pas pu etre initialisee."
      });
    } finally {
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          {tiers.map((tier, idx) => (
            ...
            >
              ...
              <div className="mb-8">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-6 ${tier.highlight ? 'bg-edu-red' : 'bg-edu-red/10'
                  }`}>
                ...
                {tier.features.map((feature, fIdx) => (
                  <div key={fIdx} className="flex items-start gap-3">
                    {feature.included ? (
                        ...
                        <X size={18} className={`${tier.highlight ? 'text-edu-bg/30' : 'text-edu-dark/30'} shrink-0 mt-0.5`} />
                    )}
                    <span className={`text-sm ${!feature.included && (tier.highlight ? 'text-edu-bg/30' : 'text-edu-dark/30')
                      }`}>
                      {feature.name}
                    </span>
                  </div>
